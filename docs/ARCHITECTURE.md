...
Isolation, Docker, and hypervisors live in [Berthos](https://github.com/hexuria/berthos). This process talks to a Berthos node only as an `EligibilityClient` (doctor attestation) and as a fulfillment URL on `kind=desktop.linux` listings.
...
| -------------------- | ------------------------- | ------------------------------------------------------- |
| `WalletPort`         | `MemoryWalletAdapter`     | `CdpWalletAdapter` (TODO — see [WALLET.md](WALLET.md)) |
| `FacilitatorPort`    | `TestFacilitator`         | `HttpFacilitator` wrapping an x402 `/verify` + `/settle` |
| `EligibilityClient`  | `MemoryEligibilityClient` | `HttpBerthosEligibilityClient` → Berthos `/doctor`     |
...
- `desktop.*` kinds require a stored Berthos doctor attestation. Missing, `ok: false`, or an unreachable doctor is a reject.
...
`TestFacilitator` accepts `test:<walletId>` signatures so CI and `npm run earn-loop` complete a spend/earn loop without a chain. A live facilitator is a drop-in `HttpFacilitator` (or `@x402/core/server` `HTTPFacilitatorClient`).
